...
function Get-ExtractedGeneratedScripts {
    ...
    if (Test-Path $safeNetPath) {
        ...
        . $safeNetPath
        $safety = Get-WgSafetyRuntimeScript -Version '15.3.2'
    }
...
function Get-FileScopedPatternMatrix {
    return @(
        ...
        @{ Files = @('lib/Install-MainSteps-0-6.ps1'); Must = @('Invoke-InstallMainSteps0to6', 'must never run in DryRun', '$script:serverIPs', '$script:serverPort', 'Remove-InstallBlocks', 'Remove-KurtarArtifacts', 'Invoke-SafeNetsh', 'blocks deferred until install completes', 'firewallpolicy blockinbound,allowoutbound', 'CustomEndpointIP requires -CustomConfig', 'Remove-IPv6FromConfig'); MustRegex = @('KS-DNS-Block.*enable=no') }
        ...
        @{ Files = @('scripts/final-line-audit.ps1'); Must = @('Get-CompleteRepoManifest', 'Add-Finding', 'Test-Ps1FileDeep', 'findings.jsonl', 'final-audit-summary.md', '15.3.2') }
    )
}
...
function Get-RoleContractMatrix {
    return @(
        ...
        @{ Role = 'emergency'; Scripts = @('enable-dns-lockdown.ps1'); Must = @('dns-lockdown-guard', 'Manual DNS', 'Administrator', 'v15.3') }
