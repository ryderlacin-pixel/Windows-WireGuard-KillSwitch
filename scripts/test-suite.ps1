...
foreach ($n in @('v13.2','13.2','$WG_KS_VERSION','safe-live-verify.ps1','oldCmd -match','powershell.exe'' OR TargetInstance.Name=''pwsh.exe','Minutes 15','Invoke-EmergencyUnbrick','EMERGENCY UNBRICK','protection stays installed','Remove-KurtarArtifacts','Invoke-DeepUnbrick','Test-MainMonitorActive','deferring reinstall','tunnel recovery delegated','WGTunnelInstallMutex','ScriptsPath','TunnelName','anti-tamper.ps1','Invoke-AntiTamperGuard','NoChainRepair','Write-GuardBackups','WGKillSwitchGuard','TaskXMLRepair','Log-Tamper','Restore-WmiSubscription','C:\ProgramData\WGKillSwitchGuard','v11.3','v11.2','WG-RebootVerify','post-reboot-verify','RebootVerifyPath','Remove-OtherMonitorProcs','v11.1','Ensure-DelayedAutoStart','Test-DelayedAutoStart','Repair-ConfigIntegrity','Repair-EssentialFirewall','Test-NetworkChanged','NetworkFingerprint','Test-BlockRulePresent','wmi-cooldown','WmiCooldownActive','Sync-KillSwitchState','Test-ServerRulePresent','Set-ServerRule','Start-HiddenScript','8.8.8.8','hits -ge 2','GPO: zombie tunnel','Test-InstallInProgress','install.inprogress','Remove-IPv6FromConfig','Install-WmiSubscription','Tunnel lost while open','60s hold','tamperTick','watchdog will deep-unbrick')) {
...
if (Test-Path $safePath) {
    ...
    foreach ($pat in @('non-disruptive', 'NEVER stops tunnel', 'Post-check: internet')) {
